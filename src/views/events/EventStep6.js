...
const EventStep6 = ({data,eventId,refreshData,allCategory,allLocation}) => {
  ...
  const [detail,setDetail] = useState(data.organizer ? data.organizer.detail : '')
  ...
  const updateData = async () =>{
    ...
    await updateOrganizer(eventId,contactData,locale).then((res)=> toast("Details updated successfully"))
    refreshData()
    setLoading(false)
}
else{
  ...
  const uploadImage = async (file)=>{
    ...
    const task =  await uploadBytes(storageRef,file).then((res)=>console.log(res.ref))
    ...
    <CardContent>
      ...
      <form>
        ...
        <Box sx={{width:'100%',textAlign:'center'}}>
            <Box sx={{position:'relative',width:100,height:100,borderRadius:'150px',overflow:'hidden',margin:'20px auto'}} >
            {
                imgSrc !== '/images/avatars/1.png' ? <Image src={imgSrc} layout='fill' /> : <Image src='/images/no-image.jpg' layout='fill' />
            }
            </Box>
            <input
                type='file'
          ...
          <Grid item xs={12} sm={6}>
            ...
            fullWidth label='Organizer email' 
            defaultValue={email} 
          ...
          <Grid item xs={12} sm={6}>
            ...
            fullWidth label='Organizer Phone number' 
            defaultValue={phone} 
          ...
          <Grid item xs={12} sm={12}>
            ...
            fullWidth label='Organizer contact details' 
            defaultValue={detail} 
